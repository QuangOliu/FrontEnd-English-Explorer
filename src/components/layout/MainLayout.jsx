...
const pathsWithWildcard = ['/exam/doing/*', '/course/*']; // Các đường dẫn có wildcard

const MainLayout = ({ children }) => {
  const location = useLocation(); // Lấy đường dẫn hiện tại

  // Kiểm tra nếu đường dẫn là một trong các path không có Navbar và Sidebar
  const isNoNavbarSidebar =
    pathsWithoutNavbarSidebar.includes(location.pathname) ||
    ...
    <>
      ...
      <Grid container style={{ flex: 1 }}>
        ...
        <Grid
          ...
          style={isNoNavbarSidebar ? {} : { padding: '20px' }}
        >
          {children}
        </Grid>
      </Grid>
